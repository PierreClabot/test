class PlanDeSalle{
  ...
  console.log("ini plan de salle");
    ...
    this.zoneMiniPlan = document.querySelector("#ZONE_MINI_PLAN");
    this.zoneMiniPlanRelief = document.querySelector("#ZONE_MINI_PLAN_RELIEF");
    ...
    this.domElement.addEventListener("wheel", e=>{
      ...
      const [x,y,originalWidth, originalHeight] = this.domElement.getAttribute("viewBox").split(" ").map(Number);

      if(e.wheelDeltaY < 0) // DEZOOM
      {

        let nh=Math.round(originalHeight * (this.echelleZoom)) 
        let nw=Math.round(originalWidth * (this.echelleZoom))

        let nx=Math.round(x+(originalWidth-nw)*(this.positionSouris.x));
        let ny=Math.round(y+(originalHeight-nh)*(this.positionSouris.y));
        this.setViewBox(`${nx} ${ny} ${nw} ${nh}`);
      }
      else // ZOOM
      {
        let nh=Math.round(originalHeight / (this.echelleZoom)) 
...
bougerEn(event,x,y)
{
  ...
  if(this.etatJeDeplace)
  {
    ...
    let largeurSVG = this.domElement.getBBox().width;  
    
    let coeff = objViewBox.largeur/largeurSVG;
    ...
    posActuelle.X-=this.vecteur.X*coeff;
    posActuelle.Y-=this.vecteur.Y*coeff;
...
limiteDeplacement(position)
{
  ...
  if(objViewBox.x < -(objViewBox.largeur/2))
  {
    posActuelle.X = -(objViewBox.largeur/2);
  }
  if(objViewBox.x > this.viewPort.largeur - (objViewBox.largeur/2))
  {
    posActuelle.X = this.viewPort.largeur - (objViewBox.largeur/2);
  }
  if(objViewBox.y < -(objViewBox.hauteur/2))
  {
    posActuelle.Y = -(objViewBox.hauteur/2);
  }
  if(objViewBox.y > this.viewPort.hauteur - (objViewBox.hauteur/2))
  {
    posActuelle.Y = this.viewPort.hauteur - (objViewBox.hauteur/2);
  }
...
setZoneMiniPlan(chaViewBox){
  let objViewBox = this.chaViewBoxVersObj(chaViewBox);
  this.zoneMiniPlan.setAttribute("x",objViewBox.x);
  this.zoneMiniPlan.setAttribute("y",objViewBox.y);
  this.zoneMiniPlan.setAttribute("width",objViewBox.largeur);
  this.zoneMiniPlan.setAttribute("height",objViewBox.hauteur);
  
  this.zoneMiniPlanRelief.setAttribute("x",objViewBox.x-10);
  this.zoneMiniPlanRelief.setAttribute("y",objViewBox.y-10);
  this.zoneMiniPlanRelief.setAttribute("width",objViewBox.largeur+20);
  this.zoneMiniPlanRelief.setAttribute("height",objViewBox.hauteur+20);
}
setViewBox(chaViewBox){
  this.domElement.setAttribute("viewBox",chaViewBox);
  this.setZoneMiniPlan(chaViewBox);
}
getViewBox(){
  return this.domElement.getAttribute("viewBox");
}
chaViewBoxVersObj(chaViewBox)
{
